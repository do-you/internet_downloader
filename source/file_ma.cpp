...
file_ma::~file_ma()
{
    ...
    if (had_init)
    {
        ...
        if (had_complete)
        {
            ...
            temp /= boost::locale::conv::utf_to_utf<wchar_t>(parms->file_name + ".abc");
            remove(temp);
        }
    }
...
std::list<block *> file_ma::init()
{
    ...
    while (++i)
    {
        ...
        bool file_exit = exists(temppath /= boost::locale::conv::utf_to_utf<wchar_t>(parms->file_name));
        bool record_exit = exists(temppath2 /= boost::locale::conv::utf_to_utf<wchar_t>(parms->file_name + ".abc"));
        if (!file_exit)//文件不存在
        {
            //直接新建
...
{
    hfile = CreateFile(path1.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, flag, FILE_ATTRIBUTE_NORMAL,
                       NULL);
    if (INVALID_HANDLE_VALUE == hfile)
        util_errno_exit("CreateFileA fail:");

    hprofile = CreateFile(path2.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, flag,
