...
size_t Internet_Downloader::add_task(string uri, list <down_parm> &parms)
{
    ...
    for (auto &x : parms) {
        ...
        if (result_count(res) == 0);//throw  invalid_argument("无效的参数:" + x.parm);
...
// 	auto temp_set = scheduler.get_activelist();
    ...
    for (auto task : temp_set) {
        ...
        per_res.files.path = (path(task->dir) / task->file_name).generic_string();
        ...
        switch (task->statues) {
            case task_info::active:
                per_res.status = "active";
                break;
            case task_info::pending:
                per_res.status = "active";
                break;
            case task_info::waitting:
                per_res.status = "waiting ";
                break;
            case task_info::pause:
                per_res.status = "paused";
                break;
            case task_info::finish:
                per_res.status = "complete";
                break;
            case task_info::fail:
                per_res.status = "error";
                break;
        }
